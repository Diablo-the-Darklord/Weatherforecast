    ...
    try:
        ...
        for forecast in forecast_list:
            ...
            if start_time <= forecast_time < end_time:
                # Group forecast data by 8-hour intervals
                interval = (forecast_time - start_time) // timedelta(hours=8)
...
if __name__ == "__main__":
    ...
    if weather_data:
        print("\nWeather Forecast for", city_name, "over the next 4 days (8-hour intervals):")
        for interval, forecasts in weather_data.items():
            interval_time = start_time + timedelta(hours=interval * 8)
